...
export class AppRestrictions extends HTMLElement {
  ...
  render() {
    ...
    <p>&nbsp;</p>
    <p><span style="font-weight: bold;">Internationalization (i18n):</span><br/>Modern frameworks offer built-in mechanisms for this. There are two approaches: 1) Implementation in the build process, as Angular does, for example. You then have a “clean” version, but also one application per language. 2) Implementation at runtime. The first option performs better at runtime. However, option 2 is perfectly adequate for most applications. And for option 2, you don't need a framework or external library: A central I18nService loads a JSON file (e.g., de.json). Components retrieve the text using a method such as t(‘button.save’). Advantage: Easy to implement.
  Disadvantage: Components must be actively informed when the language changes (events). A global observer scans the DOM for attributes such as data-i18n=“key”. When the language changes, it automatically exchanges the texts. The component itself requires hardly any logic. Alternatively, you can also restart the application to change the language, in which case you do not need events that listen for a change.
